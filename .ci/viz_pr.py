...
def main():
    # Get fonts in master branch
    subprocess.call(['git', 'checkout', 'master', 'fonts'])
    subprocess.call(['cp', '-rf', 'fonts', 'fonts_prev'])
    subprocess.call(['git', 'reset', '--hard'])
    fonts_before = glob("fonts_prev/*.ttf")

    fonts_after = get_fonts_in_pr()
    if not fonts_after:
        post_gh_msg("No fonts changed. Skipping font QA")
        return
